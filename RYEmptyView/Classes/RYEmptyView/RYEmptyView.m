...
#import "Masonry.h"
@interface RYEmptyView()

@property (nonatomic, assign) RYEmptyType type;
...
#pragma mark -- set
- (void)setType:(RYEmptyType)type {
    _type = type;
    UIImage *image;
    NSString *string;
    BOOL hideBtn = YES;
    NSBundle *bundle = [NSBundle bundleForClass:[self class]];
    NSString *filePath = [bundle pathForResource:@"empty_network.png" ofType:nil inDirectory:@"RYEmptyView.bundle"];
    //按钮 在无网络状态才显示
    switch (type) {
        case RYEmptyTypeNotNetwork:{
            image = [UIImage imageWithContentsOfFile:filePath];
            string = @"哎呀！！您的网络好像断了刷新试试吧";
            hideBtn = NO;
        }
            break;
        case RYEmptyTypeNotContent: {
            image = [UIImage imageWithContentsOfFile:filePath];
            string = @"暂无内容";
            hideBtn = YES;
        }
